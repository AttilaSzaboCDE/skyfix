...
def detection_check(alert_json, sub_id, az_tenant_id, az_client_id, az_client_secret):
    ...
    if alert_type == "firing":
        ...
        fault_type_choose(alert_reason,alert_service_type, azure_credential_sub_id, azure_credential_tenant_id,
                          azure_credential_client_id, azure_credential_secret_key)
    return 0
...
def fault_type_choose(alert_reason, alert_service_type, sub_id, az_tenant_id, az_client_id, az_client_secret):
    ...
    if alert_service_type == "vm":
        if alert_reason == "highcpu":
            ...
            vm_state, stop_result = stop_vm_by_name(resourcename, azure_credential_tenant_id, azure_credential_client_id,
                                                    azure_credential_secret_key, azure_credential_sub_id)
            if vm_state == 0:
            ...
            running_list.append(running_item)
            sleep(5)
            storage_state, stop_result = storage_attach(resourcename, azure_credential_tenant_id, azure_credential_client_id,
                                                        azure_credential_secret_key, azure_credential_sub_id)
            if storage_state == 0:
                running_list.remove(running_item)
            ...
            running_list.append(running_item)
            sleep(5)
            memory_state, stop_result = vm_scale_up(resourcename, azure_credential_tenant_id, azure_credential_client_id,
                                                    azure_credential_secret_key, azure_credential_sub_id)
            if memory_state == 0:
                running_list.remove(running_item)
            ...
            running_list.append(running_item)
            sleep(5)
            vm_state2, stop_result = vm_restart(resourcename, azure_credential_tenant_id, azure_credential_client_id,
                                                azure_credential_secret_key, azure_credential_sub_id)
            if vm_state2 == 0:
                running_list.remove(running_item)
        ...
        if alert_reason == "contdown":
            ...
            running_list.append(running_item)
            sleep(5)
            cont_state, stop_result = cont_restarting(resourcename, azure_credential_tenant_id, azure_credential_client_id,
                                                      azure_credential_secret_key, azure_credential_sub_id)
            if cont_state == 0:
                running_list.remove(running_item)
            ...
            running_list.append(running_item)
            sleep(5)
            cont_state, stop_result = cont_scale_up(resourcename, azure_credential_tenant_id, azure_credential_client_id,
                                                    azure_credential_secret_key, azure_credential_sub_id)
            if cont_state == 0:
                running_list.remove(running_item)
            ...
            running_list.append(running_item)
            sleep(5)
            cont_state, stop_result = cont_replace(resourcename, azure_credential_tenant_id, azure_credential_client_id,
                                                   azure_credential_secret_key, azure_credential_sub_id)
            if cont_state == 0:
                running_list.remove(running_item)
